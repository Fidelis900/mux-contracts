...
- [ ] `execute_with_session` / `execute_with_session_sponsored` — `session_key.require_auth()` called (plus `sponsor.require_auth()` and an allowlist check for the sponsored variant), plus revocation/expiry check against the stored `SessionKeyRecord`. **Fail-closed scope enforcement (T-40):** a key registered with an empty `scopes` list, or one that does not cover the requested `function`, is rejected with `Unauthorized` (unit tests: `test_execute_with_session_rejects_empty_scopes`, `test_execute_with_session_rejects_method_outside_scopes`). After auth and scope checks, `target.function(args)` is dispatched via `invoke_contract` under the reentrancy guard and the target's return value is forwarded to the caller (unit test: `test_execute_with_session_dispatches_to_target`).

> **Implementation note (T-40):** Both empty-scopes rejection and per-method scope matching are implemented and enforced fail-closed — a session key is rejected with `Unauthorized` unless its `scopes` list is non-empty and contains the requested `function`, checked before any state mutation. The payload-dispatch gap this note previously tracked is closed: `execute_with_session`/`execute_with_session_sponsored` now invoke `target.function(args)` rather than returning an empty value.
